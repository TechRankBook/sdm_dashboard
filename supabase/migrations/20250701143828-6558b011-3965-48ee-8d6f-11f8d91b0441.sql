...
-- Create storage buckets for vehicle files
...
CREATE POLICY IF NOT EXISTS "vehicles_insert_admin" ON public.vehicles
FOR INSERT WITH CHECK (get_user_role(auth.uid()) = 'admin');

CREATE POLICY IF NOT EXISTS "vehicles_update_admin" ON public.vehicles
FOR UPDATE USING (get_user_role(auth.uid()) = 'admin');

CREATE POLICY IF NOT EXISTS "vehicles_delete_admin" ON public.vehicles
FOR DELETE USING (get_user_role(auth.uid()) = 'admin');

-- Create RLS policies for vehicle_maintenance_logs table
